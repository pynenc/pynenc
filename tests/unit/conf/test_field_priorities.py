...
def test_config_values_map() -> None:
    ...
    conf = Config(config_values={"field": 1})
    assert conf.field == 1
...
def test_config_specific_values_map() -> None:
    ...
    conf = Config(config_values={"field": 1, "Config": {"field": 2}})
    assert conf.field == 2
...
def test_config_file() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        conf = Config(
            config_values={"field": 1, "Config": {"field": 2}},
...
def test_config_file_specific_value() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        filepath = os.path.join(tmpdir, "config.json")
        with open(filepath, mode="w") as _file:
            _file.write(json.dumps({"field": 3, "Config": {"field": 4}}))
        conf = Config(
            config_values={"field": 1, "Config": {"field": 2}},
            config_filepath=filepath,
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "PYNENC__CONFIG__FILEPATH": specific_env_filepath,
            },
        ):
            conf = Config(
                config_values={"field": 1, "Config": {"field": 2}},
