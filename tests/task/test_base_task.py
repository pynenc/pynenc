...
from pynenc.task import BaseTask
from pynenc.invocation import SynchronousInvocation, DistributedInvocation


def pytest_generate_tests(metafunc: Metafunc) -> None:
    """The test should run per each subclass of BaseTask"""
    if "app" in metafunc.fixturenames:
        metafunc.parametrize("app", BaseTask.__subclasses__(), indirect=True)


@pytest.fixture
def app(request: pytest.FixtureRequest) -> Pynenc:
    return Pynenc(task_cls=request.param)
...
def test_instanciate_task(app: Pynenc) -> None:
    ...
    assert isinstance(add, app.task_cls)
...
def test_instanciate_task_with_args(app: Pynenc) -> None:
    ...
    assert isinstance(add.options, app.task_cls.options_cls)
