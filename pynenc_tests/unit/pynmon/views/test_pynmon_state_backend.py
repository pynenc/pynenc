...
def test_state_backend_purge_handles_error(app_state_backend: "Pynenc") -> None:
    """Test that purge handles errors gracefully."""
    setup_routes()

    # Mock state backend to raise an error on purge
    mock_state_backend = MagicMock()
    mock_state_backend.purge.side_effect = RuntimeError("State purge failed")
    app_state_backend.state_backend = mock_state_backend
    ...
    with patch(
        ...
        assert "State purge failed" in data["message"]


def test_state_backend_purge_with_exception() -> None:
    """Test purge with exception from the state backend."""
    setup_routes()

    mock_app = MagicMock()
    mock_state = MagicMock()
    mock_state.purge.side_effect = Exception("Unexpected state error")
    mock_app.state_backend = mock_state

    with patch("pynmon.views.state_backend.get_pynenc_instance", return_value=mock_app):
        client = TestClient(pynmon_app)
        response = client.post("/state-backend/purge")
        ...
        data = response.json()
        assert data["success"] is False
        assert "Unexpected state error" in data["message"]
